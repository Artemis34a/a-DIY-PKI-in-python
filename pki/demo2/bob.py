...
class BobClient:
    ...
    def load_credentials(self):
        ...
        try:
            ...
            self.cert = load_cert('bob')
            self.private_key = load_key('bob', password=None)
    ...
    def load_alice_cert(self):
        """Charge le certificat d'Alice"""
        try:
            self.alice_cert = load_cert('alice')
            console.print("[green]✓ Certificat d'Alice chargé[/green]")
            return True
        except Exception as e:
            console.print(f"[red]Erreur chargement certificat Alice: {e}[/red]")
            return False
        ...
        while self.running:
            try:
                ...
                if msg_type == 'incoming_message':
                    ...
                    elif data['username'] == 'alice':
                        console.print(f"\n[yellow]⚠️ Le certificat d'Alice a été révoqué ({data['reason']})[/yellow]\n")
    ...
    def handle_incoming_message(self, data):
        ...
        console.print(Panel.fit(
            "[bold magenta]📨 NOUVEAU MESSAGE[/bold magenta]",
            border_style="magenta"
        ))

        sender = data['sender']
        console.print(f"[yellow]De:[/yellow] [magenta]{sender}[/magenta]")
    ...
    def send_encrypted_message(self, recipient, message_text):
        ...
        if recipient != 'alice':
            console.print("[red]Vous ne pouvez envoyer qu'à Alice[/red]")
            return

        if not self.alice_cert:
            console.print("[red]Certificat d'Alice non disponible[/red]")
            return

        try:
            # Vérifier si Alice est révoquée
            alice_serial = f"{self.alice_cert.serial_number:X}"
            revocation_status = check_revocation(alice_serial)

            if revocation_status['status'] == 'revoked':
                console.print(f"[red]⚠️ ATTENTION: Le certificat d'Alice est révoqué![/red]")
                console.print(f"[red]Raison: {revocation_status['reason']}[/red]")
                console.print(f"[red]Alice n'est pas fiable. Message non envoyé.[/red]")
                return
...
╔═══════════════════════════════════════════════════════════╗
║                                                           ║
║   👤 CONSOLE BOB - Messagerie Sécurisée PKI              ║
║                                                           ║
╚═══════════════════════════════════════════════════════════╝
        """, style="bold magenta")

        console.print("[magenta]Commandes disponibles:[/magenta]")
        console.print("  • Tapez votre message et appuyez sur Entrée pour l'envoyer à Alice")
        console.print("  • 'quit' ou 'exit' pour quitter")
        console.print("  • 'status' pour voir votre statut\n")

        while self.running:
            try:
                user_input = Prompt.ask("[bold magenta]Bob[/bold magenta]")
                ...
                if command in ['quit', 'exit']:
                    ...
                    # Envoyer le message à Alice
                    self.send_encrypted_message('alice', user_input)
